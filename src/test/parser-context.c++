...
struct Make_Test_Files {
	Make_Test_Files() {
		{
			std::ofstream ofs("test/tmp-parser-context-001.tmp");
			ofs << "abc 123\r\nxyz 789\n";
		}
		{
			std::ofstream ofs("test/tmp-parser-context-002.tmp");
			ofs << "abc 123\r\nxyz 789\n";
		}
	}
} _make_test_files;
...
struct Fixture {
	Context context;
	Fixture() :
		context("test/tmp-parser-context-001.tmp")
	{}
};

TEST_METHOD(Fixture, filename_is_correct) {
	EXPECT(context.filename() == "test/tmp-parser-context-001.tmp");
}
...
TEST_METHOD(Fixture, not_equal_if_files_differ) {
	Context context2("test/tmp-parser-context-002.tmp");
	EXPECT(context != context2);
}

TEST_METHOD(Fixture, fail_if_file_not_found) {
	try {
		Context context2("test/tmp-parser-context-999.tmp");
		EXPECT(false);
